...
NICHES = [
    ...
    # Питание
    "доставка еды", "кейтеринг", "торты на заказ", "кафе ресторан",
    # Образование
    ...
    # Медицина и вет
    "стоматология", "ветеринарная клиника", "медицинский центр",
    # Туризм и досуг
    "турагентство", "гостиница хостел", "экскурсии",
    # Фитнес
    "фитнес тренажерный зал", "персональный тренер йога",
    # AI-продукты (новые ниши)
    "карточки товаров WB Ozon", "логотип фирмстиль",
    "контент соцсети SMM", "рекламный ролик видео",
    "SEO тексты копирайтинг", "перевод локализация",
    "прайс меню дизайн", "telegram бот автоматизация",
    "управление репутацией отзывы", "тревел контент туризм",
    "аудит бизнеса маркетинг", "email whatsapp рассылка",
    # Маркетплейсы
    "маркетплейс продавец Wildberries", "маркетплейс продавец Ozon",
    # Строительство и недвижимость
    "агентство недвижимости", "строительство домов",
    "мебель на заказ", "кухни на заказ",
    # Детские
    "детская одежда", "детские игрушки товары",
    # Красота товары
    "косметика уход", "парфюмерия",
    # Спорт
    "спортивные товары", "спортивная секция клуб",
    # Животные
    "зоомагазин товары животных", "груминг",
    # Сезонные
    "новогодние украшения подарки", "цветы букеты",
]
    ...
    for src in sources:
        ...
        try:
            if src == "yandex":
                ...
                leads_raw, note = scan_hh(city, limit=limit)
            elif src == "vk":
                from scanner.vk_source import scan_vk
                import os as _os
                leads_raw, note = scan_vk(city, niche, limit=limit, access_token=_os.getenv("VK_TOKEN",""))
            else:
                continue
